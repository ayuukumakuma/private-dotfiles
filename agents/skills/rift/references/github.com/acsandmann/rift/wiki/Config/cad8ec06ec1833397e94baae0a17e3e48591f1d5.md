...
source_url: "https://github.com/acsandmann/rift/wiki/Config/cad8ec06ec1833397e94baae0a17e3e48591f1d5"
...
* ### Uh oh!
  ...
  There was an error while loading. [Please reload this page](https://github.com/acsandmann/rift/wiki/Config/cad8ec06ec1833397e94baae0a17e3e48591f1d5.html).
...
[Jump to bottom](https://github.com/acsandmann/rift/wiki/Config/cad8ec06ec1833397e94baae0a17e3e48591f1d5.html#wiki-pages-box)

acsandmann edited this page Aug 21, 2025
·
...
This toggles the current macos spaces activation. when rift is activated it will manage windows and do things.

## stack\_windows

`stack_windows` will stack windows within the current branch of the layout, if no joins have been made this will stack all windows in the current workspace. using `stack_windows` on an already existing stack will alter the stack orientation from horizontal->vertical or vertical->horizontal.
...
`toggle_fullscreen` will make the current window take up all of the space and go over all other windows.

## save\_and\_exit

`save_and_exit` will dump the state of rift and make it so that it can be restored when rift is started again via `rift --restore`

## serialize

`serialize` will dump all of rifts state into the console. use this when reporting a bug or trying to debug an issue.

## debug

`debug` will print the active layout tree to the console
...
`move_node` will move the current window in the specified direction (`left`, `right`, `up`, or `down`) within the current workspace

## switch\_to\_workspace

`switch_to_workspace` will switch to the specified workspace (`"Alt + B" = { switch_to_workspace = 0 }`)
...
`toggle_window_floating` will toggle the focused window between being tiled or floated

## toggle\_focus\_floating

`toggle_focus_floating` will raise all floating windows from the current workspace into view
