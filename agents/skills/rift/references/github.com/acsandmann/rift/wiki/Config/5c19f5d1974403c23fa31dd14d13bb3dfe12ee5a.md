...
source_url: "https://github.com/acsandmann/rift/wiki/Config/5c19f5d1974403c23fa31dd14d13bb3dfe12ee5a"
...
* ### Uh oh!
  ...
  There was an error while loading. [Please reload this page](https://github.com/acsandmann/rift/wiki/Config/5c19f5d1974403c23fa31dd14d13bb3dfe12ee5a.html).
...
[Jump to bottom](https://github.com/acsandmann/rift/wiki/Config/5c19f5d1974403c23fa31dd14d13bb3dfe12ee5a.html#wiki-pages-box)

acsandmann edited this page Aug 22, 2025
·
...
* `ax_role` (string): optional exact match against the macOS Accessibility AXRole (e.g., `"AXWindow"`, `"AXDialog"`).
* `ax_subrole` (string): optional exact match against the macOS Accessibility AXSubrole (e.g., `"AXSystemDialog"`).
...
All of the commands are documented below. the configuration reference will show how to associate the needed variables with commands
...
## toggle\_space\_activated

Toggle whether the focused macOS space is activated. When activated rift will manage windows in that space; when deactivated it will not.

## next\_workspace / prev\_workspace

Switch to the next / previous virtual workspace

## switch\_to\_workspace

`switch_to_workspace` will switch to the specified workspace.

## move\_window\_to\_workspace

`move_window_to_workspace` moves the focused window to the specified workspace.

## create\_workspace

Create a new virtual workspace.

## switch\_to\_last\_workspace

Switch to the last previously active workspace.

## move\_focus

`move_focus` will move focus in the specified direction (`left`, `right`, `up`, or `down`).

## move\_node

`move_node` will move the focused window/node in the specified direction (`left`, `right`, `up`, or `down`) within the current workspace/layout.

## join\_window

`join_window` will add the focused window to the branch in the specified direction (`left`, `right`, `up`, or `down`)

## stack\_windows

`stack_windows` will stack windows within the current branch of the layout; if no joins have been made this will stack all windows in the current workspace. Using `stack_windows` on an existing stack will flip orientation (horizontal ↔ vertical).

## unstack\_windows

`unstack_windows` will break the current stack.

## unjoin\_windows

`unjoin_windows` will unjoin previously joined windows (separate joined nodes).

## toggle\_window\_floating

`toggle_window_floating` will toggle the focused window between tiled and floating.

## toggle\_focus\_floating

`toggle_focus_floating` will briefly raise/bring focus to floating windows from the current workspace (handy to access floating windows quickly).

## toggle\_fullscreen

`toggle_fullscreen` will make the current window occupy the full layout area (covering other windows in that area).

## resize\_window\_grow / resize\_window\_shrink

Smartly resize the current selection. Use `resize_window_grow` to increase and `resize_window_shrink` to decrease.

## debug

`debug` prints the active layout tree to the console.

## serialize

`serialize` will dump rift's internal state to the console (useful for bug reports and debugging).
